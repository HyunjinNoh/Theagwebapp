...
import 'package:the_ajou_globe_api/client.dart';
import 'package:http/http.dart' as http;

/// {@template the_ajou_globe_api_malformed_response}
/// An exception thrown when there is a problem decoded the response body.
/// {@endtemplate}
class TheAjouGlobeApiMalformedResponse implements Exception {
  /// {@macro the_ajou_globe_api_malformed_response}
  const TheAjouGlobeApiMalformedResponse({required this.error});

  /// The associated error.
  final Object error;
}

/// {@template the_ajou_globe_api_request_failure}
/// An exception thrown when an http request failure occurs.
/// {@endtemplate}
class TheAjouGlobeApiRequestFailure implements Exception {
  /// {@macro the_ajou_globe_api_request_failure}
  const TheAjouGlobeApiRequestFailure({
...
/// {@template the_ajou_globe_api_client}
/// A Dart API client for the The Ajou Globe API.
/// {@endtemplate}
class TheAjouGlobeApiClient {
  /// Create an instance of [TheAjouGlobeApiClient] that integrates
  /// with the remote API.
  ///
  /// {@macro the_ajou_globe_api_client}
  TheAjouGlobeApiClient({
  ...
  ///
  /// {@macro the_ajou_globe_api_client}
  TheAjouGlobeApiClient.localhost({
    required TokenProvider tokenProvider,
    http.Client? httpClient,
  }) : this._(
  ...
  Future<ArticleResponse> getArticle({
    ...
    if (response.statusCode != HttpStatus.ok) {
      throw TheAjouGlobeApiRequestFailure(
        body: body,
  ...
  Map<String, dynamic> json() {
    try {
      ...
      Error.throwWithStackTrace(
        TheAjouGlobeApiMalformedResponse(error: error),
        stackTrace,
      );
    }
  }
